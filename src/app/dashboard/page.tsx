...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchDatos = async () => {
      ...
      setAlumnos(dataAlumnos.filter((a: any) => !dadosDeAlta.includes(a.id.toString())));

      // 2. Obtener la Auditoría global de la BD
      const dataAuditoria = await getAuditoriaGlobal();
  ...
  return (
    <div style={{ minHeight: '100vh', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      <header className="glass-panel" style={{ width: '100%', maxWidth: '1000px', padding: '1rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        ...
        <Link href="/" className="glass-button" style={{ padding: '8px 16px', fontSize: '0.875rem' }}>
          Cerrar Sesión
        </Link>
      </header>
